...
4. **Update State**: Mark phase as "PENDING" in ``specs/{BRANCH_NAME}/state.md``
...
5. **Update State**: Mark phase as "SKIPPED" in ``specs/{BRANCH_NAME}/state.md``
...
3. **If Restart Chosen**:
   ...
   - Mark stage as "IN PROGRESS" in ``specs/{BRANCH_NAME}/state.md``
...
4. **If Confirmed**:
   ...
   - Reset all affected stages in ``specs/{BRANCH_NAME}/state.md``
...
2. **Update Execution Plan**: Change depth level in `../../inception/workflow-planning.md`
...
3. **Update State**: Ensure ``specs/{BRANCH_NAME}/state.md`` reflects current status
...
**On Resume**:
1. **Detect Existing Project**: Check for ``specs/{BRANCH_NAME}/state.md``
...
2. **Update All Tracking**: Keep ``specs/{BRANCH_NAME}/state.md``, plan files, and `audit.md` in sync
...
flowchart TD
    ...
    Q1{"Is it current phase?"}
    A1Y["Can modify or restart<br/>current phase"]

    Q2{"Is it a completed phase?"}
    Q2A{"Assess impact on<br/>dependent phases"}
    A2Low["Modify and<br/>update dependents"]
    A2High["Recommend restart<br/>from that phase"]

    Q3{"Is it adding a<br/>skipped phase?"}
    A3Y["Check prerequisites,<br/>add to plan, execute"]

    Q4{"Is it skipping a<br/>planned phase?"}
    A4Y["Warn about impact,<br/>get confirmation, skip"]

    Q5{"Is it changing<br/>depth level?"}
    A5Y["Update plan,<br/>adjust approach"]
    A5N["Clarify request<br/>with user"]
    ...
    Q2 -->|Yes| Q2A
    Q2 -->|No| Q3
    Q2A -->|Low impact| A2Low
    Q2A -->|High impact| A2High
    Q3 -->|Yes| A3Y
    Q3 -->|No| Q4
    Q4 -->|Yes| A4Y
    Q4 -->|No| Q5
