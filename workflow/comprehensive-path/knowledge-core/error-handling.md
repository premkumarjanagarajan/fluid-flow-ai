...
**Error**: Existing ``specs/{BRANCH_NAME}/state.md`` is corrupted
...
**Scenario**: ``specs/{BRANCH_NAME}/state.md`` is corrupted or inconsistent

**Recovery Steps**:
1. Create backup: ``specs/{BRANCH_NAME}/state.md`.backup`
...
3. Reset phase status in ``specs/{BRANCH_NAME}/state.md``
...
3. Mark phase as "SKIPPED" in ``specs/{BRANCH_NAME}/state.md``
...
- **Solution**: 
  ...
  2. Check if stage was marked complete in `specs/{BRANCH_NAME}/state.md`
...
**Error**: `specs/{BRANCH_NAME}/state.md` shows stage complete but artifacts don't exist
- **Cause**: State file updated but artifact generation failed
- **Solution**:
  1. Mark stage as incomplete in `specs/{BRANCH_NAME}/state.md`
...
**Error**: Artifacts exist but `specs/{BRANCH_NAME}/state.md` shows stage incomplete
- **Cause**: Artifact generation succeeded but state update failed
- **Solution**:
  1. Verify artifacts are complete and valid
  2. Update `specs/{BRANCH_NAME}/state.md` to mark stage complete
  3. Proceed to next stage
- **Recovery**: Update state file to reflect actual completion

**Error**: Multiple stages marked as "current" in `specs/{BRANCH_NAME}/state.md`
...
- **Solution**:
  ...
  3. Correct `specs/{BRANCH_NAME}/state.md` to show single current stage
...
1. **Always validate state**: Check `specs/{BRANCH_NAME}/state.md` matches actual artifacts
