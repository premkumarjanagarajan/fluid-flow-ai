...
| Pillar | What it does |
|--------|-------------|
| **Orchestrator** | Routes every request through a staged lifecycle (Stages 0-6) |
| **Knowledge Base** | Shared rules for AI governance, security, quality, and review -- loaded before and validated after every step |
...
flowchart TD
    ...
    subgraph PostStep ["After Every Stage / Step"]
        direction LR
        SM[state-manager]
        AN[analytics]
        KB[kb-compliance]
...
2. **Post-step (compliance subagent)** -- after each step, a dedicated subagent loads the *entire* KB in its own context window, reviews the step output, and returns a PASS/FAIL verdict. This catches violations without inflating the main conversation's context.
...
Phases execute in order. Within each phase, steps execute in order. The orchestrator runs the three post-step primitives (state-manager, analytics, kb-compliance) after every step completes.
...
fluid-flow-core/
  ...
  knowledge-base-core/
    ...
    ai-governance/              -- AI operating contract, overconfidence, ADR gate, content validation
  ...
  primitives/
    ...
    kb-compliance.md            -- post-step KB compliance via subagent
  skills/
