...
Note: This clarification workflow is expected to run (and be completed) BEFORE invoking `/fasttrack.plan`. If the user explicitly states they are skipping clarification (e.g., exploratory spike), you may proceed, but must warn that downstream rework risk increases.
...
2. Resolve `SHELL_TYPE` — read the `**Shell**` field from `specs/{BRANCH_NAME}/state.md`; if missing, detect it per `../../shared/stages/shell-detection.md`. Run the appropriate script from repo root **once**:
   ...
   - If JSON parsing fails, abort and instruct user to re-run `/fasttrack.specify` or verify feature branch environment.
...
11. **Continue to next stage**: Present the next stage option to the user and wait for confirmation. When the user confirms, load and execute `./fasttrack.plan.md`.

Behavior rules:

- If no meaningful ambiguities found (or all potential questions would be low-impact), respond: "No critical ambiguities detected worth formal clarification." and suggest proceeding.
- If spec file missing, instruct user to run `/fasttrack.specify` first (do not create a new spec here).
