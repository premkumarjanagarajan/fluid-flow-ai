# Comprehensive Path Memory Loading Manifest

**This is the single source of truth for all Comprehensive Path workflow-specific memory files.**

All paths below are relative to this file's directory (`main-workflow/workflows/comprehensive/memory/`).
When loading these files, resolve each path relative to where this manifest is located.

> **Note**: Shared governance memory (AI operating contract, content validation, review gates, ISO standards, security rules, etc.) is loaded separately via the shared memory manifest at `../../shared/memory/load-shared-memory.md`. This manifest covers only the Comprehensive-specific files.

---

## Always Load

These files MUST be loaded at the start of the Comprehensive Path workflow:
