...
## MANDATORY: Comprehensive-Specific Memory Loading
**CRITICAL**: In addition to shared memory, load the Comprehensive memory manifest and follow ALL instructions within it:
- Load `../memory/load-comprehensive-memory.md` -- resolve and load every file listed, using paths relative to the manifest's location
...
- Execute **Step 3: Phase Completion Update** with the exact Comprehensive stage name after each stage
...
**Optional stage** — always prompted at the start of the Comprehensive Path. Executes only if the user opts in.

**Purpose**: Assess the engineering complexity and scope of the request before any requirements or design work begins. For large or ambiguous initiatives, recommend a decomposition into independently deliverable sub-initiatives — each assigned to the appropriate workflow (Fast-Track or Comprehensive Path).
...
After the Build and Test stage completes:

1. **Analytics: Final totals**: Execute `../../shared/stages/analytics-update.md` **Step 4: Final Totals Update** to finalise metadata, metrics, timeline, and cycle summary
...
**Next**: Once documentation is updated, the workflow proceeds to **Stage 6: Completion** (Commit, PR, Change Risk Report) as defined in `../../shared/commands/fluid-flow.md`.
```
