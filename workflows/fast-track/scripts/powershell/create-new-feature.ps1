...
if ($Help) {
    Write-Host "Usage: ./create-new-feature.ps1 [-Json] [-ShortName <name>] [-JiraTicket <ticket>] [-WithState] <feature description>"
    Write-Host ""
    Write-Host "Options:"
    ...
    Write-Host ""
    Write-Host "Branch naming pattern: {JIRA-TICKET}-{description} (e.g., GXD-1732-add-user-authentication-flow)"
    Write-Host "When -JiraTicket is omitted: {description} (e.g., add-user-authentication-flow)"
    Write-Host ""
    Write-Host "Examples:"
...
if ($JiraTicket) {
    ...
    $branchName = "$jiraClean-$branchSuffix"
} else {
...
if ($branchName.Length -gt $maxBranchLength) {
    ...
    if ($JiraTicket) {
        ...
        $maxSuffixLength = $maxBranchLength - 1 - $jiraClean.Length
    } else {
        $maxSuffixLength = $maxBranchLength
    }
    ...
    if ($JiraTicket) {
        $branchName = "$jiraClean-$truncatedSuffix"
    } else {
        $branchName = $truncatedSuffix
    }
