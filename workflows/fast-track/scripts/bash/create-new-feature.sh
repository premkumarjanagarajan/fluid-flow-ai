...
while [ $i -le $# ]; do
    ...
    case "$arg" in
        ...
        --with-state)
            ...
            ;;
        --help|-h) 
            echo "Usage: $0 [--json] [--short-name <name>] [--jira-ticket <ticket>] [--with-state] <feature_description>"
            echo ""
            echo "Options:"
            ...
            echo ""
            echo "Branch naming pattern: {JIRA-TICKET}-{description} (e.g., GXD-1732-add-user-authentication-flow)"
            echo "When --jira-ticket is omitted: {description} (e.g., add-user-authentication-flow)"
            echo ""
            echo "Examples:"
...
if [ -z "$FEATURE_DESCRIPTION" ]; then
    echo "Usage: $0 [--json] [--short-name <name>] [--jira-ticket <ticket>] <feature_description>" >&2
    exit 1
fi
...
# Build branch name: {JIRA-TICKET}-{description} (or {description} if no JIRA ticket)
if [ -n "$JIRA_TICKET" ]; then
    JIRA_CLEAN=$(echo "$JIRA_TICKET" | tr '[:upper:]' '[:lower:]')
    BRANCH_NAME="${JIRA_CLEAN}-${BRANCH_SUFFIX}"
else
...
if [ ${#BRANCH_NAME} -gt $MAX_BRANCH_LENGTH ]; then
    ...
    if [ -n "$JIRA_TICKET" ]; then
        # Account for: jira ticket + hyphen (1)
        MAX_SUFFIX_LENGTH=$((MAX_BRANCH_LENGTH - 1 - ${#JIRA_CLEAN}))
    else
        MAX_SUFFIX_LENGTH=$MAX_BRANCH_LENGTH
    fi
    ...
    if [ -n "$JIRA_TICKET" ]; then
        BRANCH_NAME="${JIRA_CLEAN}-${TRUNCATED_SUFFIX}"
    else
...
# Create state.md and audit.md if --with-state is set
if $WITH_STATE; then
...
STATEEOF
    ...
    AUDIT_FILE="$FEATURE_DIR/audit.md"
...
AUDITEOF
    ...
    PROJECT_DIR="$SPECS_DIR/_project"
    mkdir -p "$PROJECT_DIR"
fi
...
if $JSON_MODE; then
    printf '{"BRANCH_NAME":"%s","SPEC_FILE":"%s","FEATURE_NUM":"","JIRA_TICKET":"%s"}\n' "$BRANCH_NAME" "$SPEC_FILE" "$JIRA_TICKET"
else
