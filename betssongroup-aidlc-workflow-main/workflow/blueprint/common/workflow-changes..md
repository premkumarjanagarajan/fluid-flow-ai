...
3. **Update Execution Plan**: Add phase to `execution-plan.mdc` with rationale
4. **Update State**: Mark phase as "PENDING" in `aidlc-state.mdc`
5. **Execute Phase**: Follow normal phase execution process
6. **Log Change**: Document in `audit.mdc` with timestamp and reason
...
5. **Update State**: Mark phase as "SKIPPED" in `aidlc-state.mdc`
6. **Adjust Later Phases**: Note that later phases may need manual setup
7. **Log Change**: Document in `audit.mdc` with timestamp and reason
...
3. **If Restart Chosen**:
   ...
   - Mark stage as "IN PROGRESS" in `aidlc-state.mdc`
...
4. **If Confirmed**:
   ...
   - Reset all affected stages in `aidlc-state.mdc`
...
2. **Update Execution Plan**: Change depth level in `workflow-planning.mdc`
...
3. **Update State**: Ensure `aidlc-state.mdc` reflects current status
4. **Log Pause**: Document pause point in `audit.mdc`
...
**On Resume**:
1. **Detect Existing Project**: Check for `aidlc-state.mdc`
...
5. **Log Resume**: Document resume point in `audit.mdc`
...
3. **Update Unit Artifacts**:
   - Modify `unit-of-work.mdc`
   - Update `unit-of-work-dependency.mdc`
   - Revise `unit-of-work-story-map.mdc`
...
2. **Update All Tracking**: Keep `aidlc-state.mdc`, plan files, and `audit.mdc` in sync
...
3. **Log Completely**: Document full change history in `audit.mdc`
