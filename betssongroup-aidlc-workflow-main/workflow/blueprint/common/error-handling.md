...
5. **Document**: Log the error and resolution in `audit.mdc`
...
**Error**: Existing `aidlc-state.mdc` is corrupted
...
**Scenario**: `aidlc-state.mdc` is corrupted or inconsistent
...
5. Document the gap in `audit.mdc`
...
3. Reset phase status in `aidlc-state.mdc`
...
2. Document skip reason in `audit.mdc`
3. Mark phase as "SKIPPED" in `aidlc-state.mdc`
...
5. **Document Everything**: Log all decisions and changes in `audit.mdc`
